<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Kaos: {{ $kaos->merek }} - {{ $kaos->ukuran }}
        </h2>
    </x-slot>

    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('admin.kaos.update', $kaos->id_kaos) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="merek" id="merek" value="{{ old('merek', $kaos->merek) }}" required
                    ...
                    <div>
                        ...
                        <select name="tipe" id="tipe" required
                            ...
                            <option value="Lengan Panjang" {{ old('tipe', $kaos->tipe) === 'Lengan Panjang' ? 'selected' : '' }}>Lengan Panjang</option>
                            <option value="Lengan Pendek" {{ old('tipe', $kaos->tipe) === 'Lengan Pendek' ? 'selected' : '' }}>Lengan Pendek</option>
                        </select>
                        @error('tipe')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="warna_kaos" id="warna_kaos"
                            value="{{ old('warna_kaos', $kaos->warna_kaos) }}" required
                    ...
                    <div>
                        ...
                        <select name="ukuran" id="ukuran" required
                            ...
                            @foreach(['XS', 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL', '5XL'] as $size)
                                <option value="{{ $size }}" {{ old('ukuran', $kaos->ukuran) === $size ? 'selected' : '' }}>
                                    {{ $size }}</option>
                            @endforeach
                        </select>
                        @error('ukuran')
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="harga_pokok" id="harga_pokok"
                                value="{{ old('harga_pokok', $kaos->harga_pokok) }}" required min="0"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="harga_jual" id="harga_jual"
                                value="{{ old('harga_jual', $kaos->harga_jual) }}" required min="0"
                    ...
                    <div>
                        ...
                        <input type="number" name="stok_kaos" id="stok_kaos"
                            value="{{ old('stok_kaos', $kaos->stok_kaos) }}" required min="0"
                ...
                <!-- Foto -->
                <div class="mt-6">
                    <label for="foto_kaos" class="block text-sm font-medium text-gray-700 mb-2">Foto Produk</label>

                    @if($kaos->foto_kaos)
                        <div class="mb-3">
                            <p class="text-sm text-gray-600 mb-2">Foto saat ini:</p>
                            <img src="{{ asset('storage/' . $kaos->foto_kaos) }}" alt="{{ $kaos->merek }}"
